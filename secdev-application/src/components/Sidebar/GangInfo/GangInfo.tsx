...
const GangInfo: React.FC = () => {
  ...
  const group_details =
    selectedItem && selectedItem.group_details
      ? JSON.parse(selectedItem.group_details)
      : {};
  ...
  return (
    <Paper
      className={selectedItem ? 'main-paper-list' : ''}
      style={{ display: !Object.keys(group_details).length ? 'flex' : '' }}
    >
      {selectedItem &&
                  ...
                  <Paper elevation={0} className={classes.nested}>
                    <div style={{ marginBottom: '10px' }}>
                      <div className={classes.details}>
                        ...
                        {group_details[key].name}
                      </div>
                      <div className={classes.details}>
                        <span className={classes.text}>Group Size: </span>
                        {group_details[key].group_size}
                      </div>
                      <div className={classes.details}>
                        <span className={classes.text}>
                          Group Leader Name:{' '}
                        </span>
                        {group_details[key].leader_name}
                      </div>
                      <div className={classes.details}>
                        <span className={classes.text}>Group Type: </span>
                        {group_details[key].type}
                      </div>
                    </div>
                  </Paper>
                </Collapse>
